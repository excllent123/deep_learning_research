...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=11, input_channels=3, fc_factor=16):
        self.inplanes = 64
        super(ResNet, self).__init__()
        self.conv1 = weight_norm(nn.Conv2d(input_channels, 64, kernel_size=7, stride=2, padding=3,
                               bias=False))
        ...
        self.fc = weight_norm(nn.Linear(512* block.expansion * fc_factor, num_classes))
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                weight_norm(nn.Conv2d(self.inplanes, planes * block.expansion,
