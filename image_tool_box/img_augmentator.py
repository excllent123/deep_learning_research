...
def featurewise_std_normalization( x):
    '''Performs x /= std(x)'''
    img = x / (np.std(x)+1e-7)
    return img

def random_flip_axis( x, axis):
...
def random_rotation( x, rg, row_axis=0, col_axis=1, channel_axis=2,
    ...
    transform_matrix = _transform_matrix_offset_center(rotation_matrix, h, w)
    x = _apply_transform(x, transform_matrix, channel_axis, fill_mode, cval)
    return x   
...
def random_shift( x, wrg, hrg, row_axis=0, col_axis=1, channel_axis=2, 
    ...
    x = _apply_transform(x, transform_matrix, channel_axis, 
                             fill_mode, cval)
    return x    
...
def random_shear( x, intensity, row_axis=0, col_axis=1, channel_axis=2,
    ...
    h, w = x.shape[row_axis], x.shape[col_axis]
    transform_matrix = _transform_matrix_offset_center(shear_matrix, h, w)
    x = _apply_transform(x, transform_matrix, channel_axis, fill_mode, cval)
    return x    
...
def random_zoom( x, zoom_range, row_axis=0, col_axis=1, channel_axis=2,
    ...
    h, w = x.shape[row_axis], x.shape[col_axis]
    transform_matrix = _transform_matrix_offset_center(zoom_matrix, h, w)
    x = _apply_transform(x, transform_matrix, channel_axis, fill_mode, cval)
    return x
...
def random_crop( x, crop_size, sync_seed=None):
...
def _apply_transform( x, transform_matrix, channel_axis=0, fill_mode='nearest', cval=0.):
