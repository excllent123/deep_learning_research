...
class DeepQNetwork:
    def __init__(
        ...
        # set epsilon = 1, for fixed action
        ...
        # where n_feature is s , no matter dim(s)=1 or dim(s)=2
    ...
    def choose_action(self, observation, after_ep=False):
    ...
    def learn(self):
        '''
        description : 
        first replace the taget-net parameters, since it is not trainable
        ...
        q_target[np.arange(self.batch_size, dtype=np.int32), 
            batch_memory.iloc[:, self.n_features].astype(int)] = \
