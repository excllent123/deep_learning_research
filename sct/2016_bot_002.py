...
def easyVGG(img_rows,img_cols,weights_path=None):
    ...
    model.add(Dense(400, activation='softmax',init='uniform'))
    model.add(Dense(12, activation='softmax',init='uniform'))
    if weights_path:
        try :
...
# Start Training
kf = KFold(len(train_Y), n_folds=7)
for train, test in kf:
    ...
    model.fit_generator(gen_Img.flow(Tr_X, Tr_Y, batch_size=32),
                    samples_per_epoch=len(Tr_X), nb_epoch=100, validation_data=(Te_X, Te_Y),callbacks=callbacks_list)
