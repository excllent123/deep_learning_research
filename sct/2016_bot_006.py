...
model = VGG_16(img_rows,img_cols,weights_path='../hub/model/{}1.h5'.format(model_name))
...
for train, test in kf:
    ...
    model.fit_generator(gen_Img.flow(Tr_X, Tr_Y, batch_size=batchSize),
                    samples_per_epoch=len(Tr_X), nb_epoch=20, validation_data=gen_Img.flow(Te_X, Te_Y,batch_size=batchSize),nb_val_samples=Te_X.shape[0],callbacks=[remote])
